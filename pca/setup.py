"""
Setup script for PCA (Python Coverage Agent)
"""
    ...
    def install_hooks(self):
        ...
        try:
            ...
            if not site_packages_path.exists():
                print(f"[PCA] Warning: site-packages directory does not exist: {site_packages_path}")
                return
            ...
            # 获取pca包的安装路径
            # 在安装后，pca包应该在site_packages/pca
            pca_package_path = site_packages_path / "pca"
            
            # 写入.pth文件，内容是pca包的路径
            with open(pth_file, 'w') as f:
                f.write(str(pca_package_path) + '\n')
            
            print(f"[PCA] Created .pth file: {pth_file}")
            
            # 2. 创建 sitecustomize.py 文件
            sitecustomize_file = site_packages_path / "sitecustomize.py"
            
            # 读取模板
            sitecustomize_template = Path(__file__).parent / "pca" / "sitecustomize.py"
            if sitecustomize_template.exists():
                # 复制sitecustomize.py到site-packages
                import shutil
                shutil.copy2(sitecustomize_template, sitecustomize_file)
                print(f"[PCA] Created sitecustomize.py: {sitecustomize_file}")
            else:
                # 如果模板不存在，创建一个简单的版本
...
'''
            ...
            print("[PCA] Installation hooks installed successfully!")
        
        except Exception as e:
            print(f"[PCA] Warning: Failed to install hooks: {e}")
            import traceback
            traceback.print_exc()
...
setup(
    ...
    version="0.1.0",
    description="PCA (Python Coverage Agent) - 业务无侵入的Python覆盖率上报插件",
